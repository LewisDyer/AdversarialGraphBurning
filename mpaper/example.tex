...
Consider a partial colouring of $G$ as produced by the above procedure with red clusters and the forest of unburned vertices $F$ comprised of a set of trees $\mathcal{T}$. Then the maximum number of red clusters on $F$ is bounded above by $$\max_{T \in \mathcal{T}} rd(T)$$
\end{corollary}
