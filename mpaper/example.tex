...
Note that in general this caterpillar is not unique
...
We mark that subtracting 2 from the caterpillar distance is necessary in order to remove the endpoints of this graph from the total. Once this notion of distance to a caterpillar endpoint is defined, the maximal nature of this caterpillar implies constraints on the size of trees in the resulting forest $\mathcal{F}$:
...
Consider a partial colouring of $G$ as produced by the algorithm in Definition \ref{def/caterpillar-algorithm}, with a maximal caterpillar of length $L$ (hence $G$ has diameter $L$), and the forest of unburned vertices $F$ made up of a set of trees $\mathcal{T}$. Then the maximum number of red clusters on $F$ is at most $\lfloor \frac{L}{2} - 1 \rfloor$.
